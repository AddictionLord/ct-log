...
def visualize_patch_features(patch_features: torch.Tensor, original_image: torch.Tensor, num_components: int = 6) -> None:
    """
    ...
    # Show original image
    axes[0, 0].imshow(original_denorm.permute(1, 2, 0))
...
def analyze_feature_similarity(patch_features: torch.Tensor, original_image: torch.Tensor) -> None:
    ...
    for i, ref_patch in enumerate(reference_patches):
        ...
        axes[0, i].imshow(original_denorm.permute(1, 2, 0))
        ...
        rect = plt.Rectangle(
            (ref_col * patch_w, ref_row * patch_h),
            patch_w, patch_h,
        ...
        # Show similarity map
        similarity_map = similarity_matrix[ref_patch].reshape(patch_size, patch_size).cpu().numpy()
