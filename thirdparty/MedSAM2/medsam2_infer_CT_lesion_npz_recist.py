...
parser.add_argument("--propagate_with_box", default=False, action="store_true", help="whether to propagate with box")
parser.add_argument("--save_nifti", default=False, action="store_true", help="whether to save nifti")
parser.add_argument("--save_overlay", default=False, action="store_true", help="whether to save the overlay image")
parser.add_argument(
    "--num_workers",
    type=int,
    default=1,
...
model_cfg = "//" + join(
    script_directory, model_cfg
)  # add additional one '/' in front of the path to avoid issues with relative paths
...
if checkpoint == "medsam2_recist":
    ckpt_path = hf_hub_download(
        repo_id="wanglab/MedSAM2", filename="medsam2_FLARE25_RECIST_baseline.pt", cache_dir="./checkpoints"
    )
...
pathlib.Path(pred_save_dir).mkdir(exist_ok=True, parents=True)
if save_overlay:
    pathlib.Path(png_save_dir).mkdir(exist_ok=True, parents=True)
if save_nifti:
    pathlib.Path(nifti_path).mkdir(exist_ok=True, parents=True)
...
def infer_3d(img_npz_file):
    ...
    for j, ulab in enumerate(unique_labs):
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            frame_idx, object_ids, masks = predictor.add_new_mask(
                inference_state, frame_idx=z_mid, obj_id=1, mask=mask_prompt
            )
            segs_3D[z_mid, ((masks[0] > 0.0).cpu().numpy())[0]] = idx
            for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(
                inference_state, start_frame_idx=z_mid, reverse=False
            ):
            ...
            for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(
                inference_state, start_frame_idx=z_mid, reverse=True
            ):
                segs_3D[(z_min_per_lab + out_frame_idx), (out_mask_logits[0] > 0.0).cpu().numpy()[0]] = idx
            predictor.reset_state(inference_state)
    ...
    if save_overlay:
        ...
        if not propagate_with_box:
            ...
            if last_pts.size > 0:
                ax.scatter(
                    last_pts[:, 0], last_pts[:, 1], s=20, c="red", marker="o", edgecolors="white", linewidths=0.5
                )
